...
export const createPost = async (
  ...
  try {
    ...
    for (let i of media) {
      ...
      if (
        !/^http:\/\/localhost:5173\/media\/[a-zA-Z0-9_-]+\.(jpeg|jpg|png|gif|webp|mp4)$/i.test(
          i
        )
      )
        throw "Invalid image/video URL";
    }
    ...
    for (let j of category) {
      ...
      if (!["Adventure", "Cultural Experiences", "Leisure"].includes(j))
